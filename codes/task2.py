...
class TID(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        for char in text:
            if char in cti:
                some.append(cti[char])

        some += [0] * (MAX_TEXT_LEN - len(some))  

        return image, torch.tensor(some)
...
tdata, vdata, testdata = random_split(dataset, [tsize, vsize, testsize])


tload = DataLoader(tdata, batch_size=20, shuffle=True)
vload = DataLoader(vdata, batch_size=20, shuffle=False)
...
with torch.no_grad():
    for images, labels in testload:
        ...
        for i in range(outputs.size(1)):  
                ...
                predictions.append((prediction, realword))
                total_chars += len(realword)
                for a, b in zip(prediction, realword):
                    if a == b:
                        correct_chars += 1
